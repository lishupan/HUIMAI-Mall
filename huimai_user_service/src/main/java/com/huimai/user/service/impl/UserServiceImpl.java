...
import com.huimai.mapper.TbUserMapper;
import com.huimai.pojo.TbUser;
import com.huimai.pojo.TbUserExample;
import com.huimai.pojo.TbUserExample.Criteria;
...
public class UserServiceImpl implements UserService {
	...
	String smscode=	(long)(Math.random()*1000000)+"";
		...
		//2、保存验证码到redis
		redisTemplate.boundValueOps(mobile).set(smscode,5, TimeUnit.MINUTES);
	...
	public boolean checkSmsCode(String mobile, String UserSmscode) {
		//从redis缓存读取指定手机号码，验证码
	String redisSmsCode= (String) redisTemplate.boundValueOps(mobile).get();
	//判断redis缓存是否读取到验证码
		if(redisSmsCode==null){
			return false;
		}else {
			//比对redis验证码e和用户输入的验证码是否一致
